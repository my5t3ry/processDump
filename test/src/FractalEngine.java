 ...
 * since: 26.08.17
 */
public class FractalEngine extends PApplet {

    public static void main(String... args) {
        FractalEngine main = new FractalEngine();
...
//================================= global vars
    ...
    int _maxlevels = 4;
    float _strutFactor = 0.3f;
    float _noiseoff = 2.0f;
    ...
    class Branch {
        ...
        void init() {
            ...
            for (int j = 0; j < hpArr.length; j++) {
                ...
                int nextj = j + hpArr.length / 2 + 1;

                if (nextj >= hpArr.length) {
                    nextj -= hpArr.length;
                }
        ...
        void drawme() {
            stroke(color(0,255,0), level * 8.2f);
            for (int k = 0; k < ppArr.length; k++) {
                int nextk = k + 1;
                if (nextk == ppArr.length) {
                    nextk = 0;
                }
