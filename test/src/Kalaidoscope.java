 ...
 * since: 26.08.17
 */
public class Kalaidoscope extends PApplet {

    public static void main(String... args) {
        Kalaidoscope main = new Kalaidoscope();
...
//================================= global vars
    ...
    void launch(float ex, float why) {
        ...
        flock = new Flock();
        for (int i = 0; i < 250; i++) {
            flock.addBoid(new Boid(new Vector3D(ex, why), 2.0f, 0.05f));
        }
    }
    ...
    void mirror() {
        ...
        loadPixels();
        for (int yyy = 1; yyy < height; yyy++) {
            for (int xxx = 1; xxx < width / 2 + 1;
            xxx++){
                pixels[(width - xxx) + (yyy * width)] = pixels[xxx + (yyy * width)];
            }
        }
        updatePixels();
    }
...
//================================= boids objects (c/o processing.or)
